...
def page_home():
    ...
    <div class="fu1" style="margin-bottom:2rem;">
      ...
      <div style="font-size:14px;color:#6B7280;line-height:1.7;max-width:580px;">
        Financial distress occurs when a company can no longer meet its financial obligations — a risk that can affect investors, creditors, and stakeholders.
        DistressIQ helps analysts, investors, and researchers assess that risk for any publicly listed company using
        multiple academic models, because no single model tells the whole story.
      </div>
    </div>
    """, unsafe_allow_html=True)

    # ── CORE MODELS ────────────────────────────────────────────────────────────
    st.markdown('<div class="sec-hdr fu2"><span class="sec-lbl">Core Models</span><div class="sec-line"></div></div>', unsafe_allow_html=True)
    col1, col2 = st.columns(2, gap="medium")

    with col1:
        st.markdown("""
        <div class="model-card fu3">
        ...
        st.markdown("""
        <div class="model-card fu5" style="margin-top:1rem;">
          <div class="model-card-header">
            <div style="display:flex;align-items:center;gap:8px;">
    ...
    with col2:
        ...
        ml_badge_color = "#A855F7" if _ml_available() else "#9CA3AF"
        ml_badge_bg    = "rgba(168,85,247,0.1)" if _ml_available() else "rgba(156,163,175,0.1)"
        ml_dot_color   = "#D946EF" if _ml_available() else "#9CA3AF"
        st.markdown(f"""
        <div class="model-card fu6" style="margin-top:1rem;">
          <div class="model-card-header">
            <div style="display:flex;align-items:center;gap:8px;">
              <div class="model-card-dot" style="background:{ml_dot_color};"></div>
    ...
    # ── HOW TO USE ─────────────────────────────────────────────────────────────
    st.markdown('<div class="sec-hdr" style="margin-top:2rem;"><span class="sec-lbl">How to Use This App</span><div class="sec-line"></div></div>', unsafe_allow_html=True)
    st.markdown("""
    <div class="qs-box">
      <div class="qs-step"><div class="qs-num">1</div>
        <div><b style="color:#1E1B4B;">Pick a model</b> from the sidebar on the left — start with Altman Z-Score if you are unsure</div></div>
      ...
      <div class="qs-step"><div class="qs-num">3</div>
        <div><b style="color:#1E1B4B;">Compare results across models</b> — use the Comparison tool to run multiple models on the same company side by side</div></div>
    </div>
    """, unsafe_allow_html=True)

    # ── MODEL GUIDE ────────────────────────────────────────────────────────────
    st.markdown('<div class="sec-hdr" style="margin-top:2rem;"><span class="sec-lbl">Model Reference</span><div class="sec-line"></div></div>', unsafe_allow_html=True)
    st.markdown(
        ...
        '<div style="font-size:11px;color:#6B7280;line-height:1.5;">Combines 5 accounting ratios to classify companies into Safe, Grey, or Distress zones.</div>'
        '</div>'
        ...
        '<div style="font-size:11px;color:#6B7280;line-height:1.5;">Uses 9 variables in a logistic model to output a direct probability of financial distress.</div>'
        '</div>'

        '<div style="padding:10px 12px 10px 0;border-bottom:1px solid #F3F2FC;border-right:1px solid #F3F2FC;">'
        '<div style="font-size:12px;font-weight:700;color:#1E1B4B;margin-bottom:3px;">Zmijewski Score</div>'
        '<div style="font-size:11px;color:#6B7280;line-height:1.5;">A probit model using ROA, leverage, and liquidity to estimate one-year distress probability.</div>'
        '</div>'

        '<div style="padding:10px 0 10px 16px;border-bottom:1px solid #F3F2FC;">'
        '<div style="font-size:12px;font-weight:700;color:#1E1B4B;margin-bottom:3px;">ML Distress Score</div>'
        '<div style="font-size:11px;color:#6B7280;line-height:1.5;">An XGBoost classifier trained on 35 years of Compustat data, combining all model signals.</div>'
        '</div>'
        ...
        '<div style="font-size:11px;color:#6B7280;line-height:1.5;">Scores financial health from 0–9 across profitability, leverage, and operating efficiency.</div>'
        '</div>'
        ...
        '<div style="font-size:11px;color:#6B7280;line-height:1.5;">Detects the likelihood of earnings manipulation using 8 accounting indices.</div>'
        '</div>'
    ...
    if btn1 and t1.strip():
        ...
        if not d["ok"]:
            ...
            if PLOTLY_AVAILABLE and trend_z:
                ...
                _fig_trend = pgo.Figure()
                _fig_trend.add_trace(pgo.Bar(
                    x=trend_z["years"], y=trend_z["revenue"],
                    name="Revenue", marker_color="#DDD8FA",
                    marker_line=dict(color="#B8B0F5", width=1),
                    text=[f"${v:.1f}B" if v is not None else "" for v in trend_z["revenue"]],
                    textposition="outside", textfont=dict(size=10, color="#6B7280"),
                ))
                _ebit_clean = [v for v in trend_z["ebit"] if v is not None]
                if _ebit_clean:
                    _fig_trend.add_trace(pgo.Scatter(
                        x=trend_z["years"], y=trend_z["ebit"],
